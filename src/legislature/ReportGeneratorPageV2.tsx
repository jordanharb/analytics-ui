...
interface PersonSearchResult extends Person {
...
const ReportGeneratorPageV2: React.FC = () => {
  ...
  const searchCachedPeople = async (term: string) => {
    try {
      // Prefer mv-based RPC for better results
      const { data, error: rpcError } = await supabase.rpc('search_people_with_sessions', { p_search_term: term });
      if (rpcError) throw rpcError;
      // Use results as-is from the mv-backed RPC
      const items: PersonSearchResult[] = (data || []).slice(0, MAX_AUTOCOMPLETE);
