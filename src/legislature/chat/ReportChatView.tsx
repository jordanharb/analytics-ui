...
const ToolExecutionProgress: React.FC<{ isVisible: boolean; currentTool?: string }> = ({
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg mx-4 mb-4">
      <div className="flex items-center gap-2">
        <div className="w-4 h-4 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
        <span className="text-sm font-medium text-indigo-800">
          {currentTool ? `Analyzing: ${currentTool}` : 'Processing campaign finance data...'}
        </span>
      </div>
      <div className="text-xs text-indigo-600">
        Campaign finance analysis may require multiple database queries
      </div>
    </div>
  );
};
...
export const ReportChatView: React.FC<ReportChatViewProps> = ({ reportContent, reportTitle }) => {
  ...
  const {
    ...
    api: '/api/chat',
    body: {
      reportContext: `You just generated this report${reportTitle ? ` titled "${reportTitle}"` : ''}. The user may ask you questions about it.
          ...
          {messages.length === 0 && !isLoading && (
            <div className="max-w-2xl mx-auto text-center py-12">
              <div className="card card-elevated p-8">
                <h3 className="text-lg font-semibold mb-4">Campaign Finance Analysis</h3>
                <p className="text-gray-600 mb-4">Ask me about donors, legislators, bills, and campaign contributions:</p>
                <ul className="text-left text-gray-600 mb-6 space-y-2">
                  <li className="flex items-start">
                    <span className="mr-2">🏛️</span>
                    Search for legislators and candidates
                  </li>
                  <li className="flex items-start">
                    <span className="mr-2">💰</span>
                    Find donors and campaign contributions
                  </li>
                  <li className="flex items-start">
                    <span className="mr-2">📜</span>
                    Analyze voting records and bill sponsorships
                  </li>
                  <li className="flex items-start">
                    <span className="mr-2">🔍</span>
                    Discover conflicts of interest patterns
                  </li>
                </ul>
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  <button
                    className="chip"
                    onClick={() => handleInputChange({ target: { value: "Who are the top donors to John Smith?" } } as React.ChangeEvent<HTMLTextAreaElement>)}
                  >
                    "Find top donors"
                  </button>
                  <button
