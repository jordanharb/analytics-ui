...
const EntityDetailView: React.FC<EntityDetailViewProps> = ({ entityId }) => {
  const normalizedEntityId = Number.isFinite(entityId) ? Math.trunc(entityId) : NaN;
  ...
  const loadEntityData = async () => {
    ...
    try {
      ...
      const { data: entityData, error: entityError } = await supabase
        .rpc('get_entity_details', { p_entity_id: normalizedEntityId });

      if (entityError) {
        console.error('Error loading entity details:', entityError);
      }
      ...
      const { data: financialData, error: financialError } = await supabase
        .rpc('get_entity_financial_summary', { p_entity_id: normalizedEntityId });
      ...
      const { data: statsData, error: statsError } = await supabase
        .rpc('get_entity_summary_stats', { p_entity_id: normalizedEntityId });

      if (statsError) {
        console.error('Error loading summary stats:', statsError);
      }
      ...
      const { data: reportsData, error: reportsError } = await supabase
        .rpc('get_entity_reports', { p_entity_id: normalizedEntityId });
  ...
  const loadMoreTransactions = async (initial = false) => {
    if (!Number.isFinite(normalizedEntityId) || normalizedEntityId <= 0) return;
  ...
  const downloadAllTransactions = async () => {
    ...
    try {
      ...
      if (allTx) {
        ...
        downloadCSV(mappedData, `transactions_${normalizedEntityId}.csv`, [
          { key: 'Date', label: 'Date' },
  ...
  const downloadAllDonations = () => {
    const mappedData = donations.map(don => ({
      ...
      Report: don.report_name
        ...
        downloadCSV(mappedData, `donations_${normalizedEntityId}.csv`, [
      { key: 'Date', label: 'Date' },
      { key: 'Amount', label: 'Amount' },
  ...
  const downloadAllReports = () => {
    const mappedData = reports.map(report => ({
      ...
      IsAmended: report.is_amended ? 'Yes' : 'No'
        ...
        downloadCSV(mappedData, `reports_${normalizedEntityId}.csv`, [
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>
          {entity.primary_committee_name || entity.primary_candidate_name || 'Unknown Entity'}
        </h1>
        {entity.primary_candidate_name && entity.primary_committee_name && (
          <h2 style={{ fontSize: '1.2rem', fontWeight: 'normal', marginBottom: '0.5rem', color: '#666' }}>
            {entity.primary_candidate_name}
          </h2>
        )}
