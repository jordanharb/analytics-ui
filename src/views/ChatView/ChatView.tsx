...
import type { Message as HistoryMessage } from '../../lib/chat/ChatHistoryManager';
import type { ProviderType, ModelInfo } from '../../lib/mcp/providers/types';
import type { Message } from 'ai';
import { AISDKChatView } from './AISDKChatView';

const LegacyChatView: React.FC = () => {
...
// Helper to create compatible Message objects
...
// Convert between Message types for compatibility
const toHistoryMessage = (msg: Message): HistoryMessage => ({
  id: msg.id,
  role: msg.role as 'user' | 'assistant' | 'system',
  content: typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content),
  timestamp: msg.createdAt instanceof Date ? msg.createdAt.toISOString() : new Date(msg.createdAt || Date.now()).toISOString(),
});

const fromHistoryMessage = (msg: HistoryMessage): Message => ({
  id: msg.id,
  role: msg.role,
  content: msg.content,
  ...
  useEffect(() => {
    ...
    if (!historyManagerRef.current) {
      ...
      setMessages(current.messages.map(fromHistoryMessage));
    }
  ...
  const handleSendMessage = async (content: string) => {
    ...
    // Save to history
    historyManagerRef.current?.addMessage(toHistoryMessage(userMessage));
    ...
    try {
      ...
      setMessages(prev => {
        ...
        if (lastMessage && lastMessage.role === 'assistant') {
          historyManagerRef.current?.addMessage(toHistoryMessage(lastMessage));
        }
        return prev;
      });
    }
  ...
  const handleSelectSession = (sessionId: string) => {
    if (historyManagerRef.current?.setCurrentSession(sessionId)) {
      ...
      if (session) {
        setCurrentSession(session);
        setMessages(session.messages.map(fromHistoryMessage));
      ...
      <div className="flex-1 flex flex-col max-w-6xl mx-auto w-full h-full overflow-hidden">
        ...
        <div className="flex-shrink-0 border-t border-gray-200 px-6 py-4">
          <ChatInput
            ...
            canSend={!isStreaming && !!mcpClientRef.current}
          />
        </div>
      </div>
    </div>
