...
BEGIN
  ...
  FROM (
    SELECT
      REPLACE(tag_value, '_', ' ') as inst_name,
      COUNT(*) as cnt
    FROM v2_events e
  ...
  FROM (
    SELECT
      REPLACE(tag_value, 'Church:', '') as church_name,
      COUNT(*) as cnt
    FROM v2_events e
    CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(e.category_tags, '[]'::jsonb)) AS tag_value
    WHERE
  ...
  -- Top category tags (parent categories)
  ...
  -- Note: Use consistent key names matching frontend interface
